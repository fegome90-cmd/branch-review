...
export function detectSensitiveZones(changedFiles: string[]): SensitiveZone[] {
  const zones: Map<
    string,
    { files: string[]; riskLevel: 'HIGH' | 'MEDIUM' | 'LOW' }
  > = new Map();
  ...
  const patterns: Array<{
    ...
    {
      pattern: /auth|login|password|token|session|credential/i,
      zone: 'Authentication',
      risk: 'HIGH',
    },
    {
      pattern: /schema\.prisma|migration|\.sql$/i,
      zone: 'Database/Schema',
      risk: 'HIGH',
    },
    {
      pattern: /api|route|controller|endpoint/i,
      zone: 'API Endpoints',
      risk: 'MEDIUM',
    },
    {
      pattern: /\.env|config|settings/i,
      zone: 'Configuration',
      risk: 'MEDIUM',
    },
    {
      pattern: /middleware|guard|interceptor/i,
      zone: 'Security',
      risk: 'HIGH',
    },
...
export function determineThirdAgent(
  stack: StackInfo,
  ...
  const hasSqlChanges = sensitiveZones.some(
    (z) => z.zone === 'Database/Schema',
  );
...
export function determineReviewType(
  ...
  const hasDbSensitiveZone = sensitiveZones.some(
    (z) => z.zone === 'Database/Schema',
  );

  const hasPython =
    hasPythonInStack && (hasPythonChanges || changedFiles.length === 0);
  const hasSql =
    hasSqlFileChanges || hasSchemaOrMigrationChanges || hasDbSensitiveZone;
