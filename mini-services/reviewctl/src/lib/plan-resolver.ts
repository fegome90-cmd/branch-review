...
import { getCurrentBranch, getBaseBranch, getCurrentSha } from './utils.js';
...
export async function resolvePlan(): Promise<{ status: PlanStatus; path: string | null; candidates?: PlanCandidate[] }> {
  ...
  if (fs.existsSync(DOCS_DIR)) {
    ...
    for (const file of files) {
      const score = scorePlan(file, branchIdentifiers);
      if (score >= 50) { // Strong match threshold
        candidates.push({
  ...
  if (fs.existsSync(allowlistPath)) {
    allowedPlans = fs.readFileSync(allowlistPath, 'utf-8')
      .split('\n')
      .map(line => line.trim())
