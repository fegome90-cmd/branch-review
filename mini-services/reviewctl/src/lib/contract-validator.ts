...
const REQUIRED_SECTIONS = {
  summary: /^##\s*Summary/mi,
  findings_p0: /^###?\s*P0|####?\s*P0-/mi,
  findings_p1: /^###?\s*P1|####?\s*P1-/mi,
  findings_p2: /^###?\s*P2|####?\s*P2-/mi,
  test_plan: /^##\s*(Test\s*Plan|Tests?)/mi,
  confidence: /^##\s*Confidence|Confidence:\s*\d/mi,
  verdict: /^##\s*Verdict/mi
};

// Evidence pattern: file:line or file(line) or `file:line`
const EVIDENCE_PATTERN = /(?:`([^`]+:\d+)`|([a-zA-Z0-9_\-./]+\.\w+:\d+)|\*\*Location\*\*[^\n]*(\S+:\d+))/gi;
...
export function validateReport(content: string): ValidationResult {
  ...
  if (lineCount > MAX_REPORT_LINES) {
    errors.push(`Report exceeds ${MAX_REPORT_LINES} lines (actual: ${lineCount})`);
  }
  ...
  const hasFindings = sections.findings_p0 || sections.findings_p1 || sections.findings_p2;
  if (!hasFindings) {
    // Check if report explicitly states "No findings"
    const noFindingsPattern = /no\s+(P0|P1|P2|critical|blocking|findings?|issues?)/i;
    const hasNoFindings = noFindingsPattern.test(content);
    if (!hasNoFindings) {
      warnings.push('No findings sections found (P0/P1/P2). If intentional, state "No findings" explicitly.');
    }
  }
  ...
  if (findingsWithoutEvidence.length > 0) {
    for (const finding of findingsWithoutEvidence) {
      warnings.push(`Finding "${finding.title}" lacks proper evidence (file:line format)`);
    }
  }
...
function extractStatistics(content: string): { p0_count: number; p1_count: number; p2_count: number } {
  ...
  return {
    p0_count: p0TableMatch ? parseInt(p0TableMatch[1]) : (p0Matches?.length || 0),
    p1_count: p1TableMatch ? parseInt(p1TableMatch[1]) : (p1Matches?.length || 0),
    p2_count: p2TableMatch ? parseInt(p2TableMatch[1]) : (p2Matches?.length || 0)
  };
}
...
function checkFindingsEvidence(content: string): FindingInfo[] {
  ...
  while ((match = findingPattern.exec(content)) !== null) {
    ...
    const endIndex = nextHeading > 0 ? Math.min(nextHeading, nextSection > 0 ? nextSection : content.length) : (nextSection > 0 ? nextSection : content.length);
    
    const findingContent = content.substring(startIndex, endIndex);
    
    // Check for evidence
    const hasEvidence = EVIDENCE_PATTERN.test(findingContent) || 
