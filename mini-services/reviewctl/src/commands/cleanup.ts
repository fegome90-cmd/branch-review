...
export async function cleanupCommand(options: { all?: boolean; olderThan?: string }) {
  ...
  try {
    ...
    if (options.all) {
      ...
      try {
        ...
        const mergedBranches = execSync(`git branch --merged ${baseBranch}`, { encoding: 'utf-8' });
        canDelete = mergedBranches.includes(branch) || !branch.startsWith('review/');
      } catch {
      ...
      if (canDelete) {
        ...
        try {
          ...
          const reviewSessions = sessions.split('\n')
        ...
        spinner.warn(chalk.yellow('Branch not merged. Use --force to clean anyway.'));
        console.log(chalk.gray(`  Switch to base branch and merge first: git checkout ${run.base_branch}`));
      }
