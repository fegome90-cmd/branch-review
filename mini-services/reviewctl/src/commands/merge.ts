...
export async function mergeCommand(options: { squash?: boolean; force?: boolean }) {
  const spinner = ora('Preparing merge...').start();
  
  try {
    // Check we're on a review branch
    if (!isOnReviewBranch()) {
      spinner.fail('Not on a review/* branch. Switch to a review branch first.');
      process.exit(1);
    }
    ...
    if (finalJson.verdict !== 'PASS' && !options.force) {
      spinner.fail('Cannot merge: verdict is FAIL. Fix P0 issues first or use --force.');
      process.exit(1);
    }
    ...
    try {
      ...
      spinner.fail('Working tree has uncommitted changes. Commit or stash first.');
      process.exit(1);
    }
    ...
    try {
      const prResult = execSync('gh pr view --json number,state 2>/dev/null', { encoding: 'utf-8' });
    ...
    try {
      if (options.squash) {
        ...
        execSync(`git commit -m "Merge review: ${run.branch}"`, { stdio: 'inherit' });
      } else {
        execSync(`git merge ${run.branch} --no-ff -m "Merge review: ${run.branch}"`, { stdio: 'inherit' });
      }
    } catch (error) {
